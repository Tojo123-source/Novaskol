@php
    $mobileState = $selectedContact && $conversationId ? 'mobile-chat-open' : 'mobile-chat-list';
@endphp
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Chat prive - {{ $ecole->nom ?? 'Ecole' }}</title>
    ...
    <style>
        .chat-shell { display:grid; grid-template-columns:310px minmax(0,1fr); gap:16px; height:calc(100vh - 130px); min-height:560px; }
        .chat-panel { background:var(--card); border:1px solid var(--border); border-radius:8px; overflow:hidden; min-height:0; position:relative; }
        .conversation-panel { display:flex; flex-direction:column; }
        .contact-search { padding:14px; border-bottom:1px solid var(--border); }
        .contact-search input,.composer input[type="text"] { width:100%; padding:12px; border-radius:8px; border:1px solid var(--border); background:var(--surface); color:var(--text); }
        .contacts { overflow:auto; height:calc(100% - 66px); }
        .contact { display:flex; gap:11px; align-items:center; padding:12px 14px; color:var(--text); text-decoration:none; border-bottom:1px solid rgba(255,255,255,.04); }
        .contact:hover,.contact.active { background:rgba(0,200,83,.12); }
        .avatar { width:42px; height:42px; object-fit:cover; border-radius:8px; border:1px solid var(--border); background:#0f172a; flex:0 0 auto; }
        .contact-main { min-width:0; flex:1; }
        .contact-main strong { display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .muted { color:var(--text-sec); font-size:.86rem; }
        .status-dot { width:10px; height:10px; border-radius:999px; display:inline-block; border:2px solid var(--card); background:#64748b; }
        ...
        .chat-head .profile-trigger { display:flex; align-items:center; gap:12px; background:transparent; border:0; color:var(--text); cursor:pointer; text-align:left; flex:1; min-width:0; overflow:hidden; padding:0; }
        ...
        .chat-head-copy strong { max-width:320px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .chat-head-copy .muted { display:inline-flex; align-items:center; gap:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        ...
        .bubble { max-width:min(680px,82%); padding:11px 13px; border-radius:18px; background:#1f2937; border:1px solid var(--border); align-self:flex-start; }
        ...
        .bubble-meta { margin-top:6px; font-size:.76rem; color:var(--text-sec); display:flex; justify-content:flex-end; gap:8px; align-items:center; }
        ...
        .bubble img.chat-image { max-width:260px; max-height:220px; border-radius:12px; display:block; margin-top:8px; border:1px solid var(--border); cursor:pointer; }
        .file-link { display:inline-flex; align-items:center; gap:8px; color:#bfdbfe; margin-top:8px; text-decoration:none; font-weight:700; cursor:pointer; }
        ...
        .file-btn,.send-btn { border:0; border-radius:999px; padding:12px 15px; cursor:pointer; font-weight:800; color:white; display:grid; place-items:center; }
        ...
        .profile-modal { width:min(440px,100%); background:var(--card); border:1px solid var(--border); border-radius:8px; padding:22px; box-shadow:0 20px 50px rgba(0,0,0,.45); }
        .profile-top { display:flex; gap:14px; align-items:center; margin-bottom:16px; }
        .profile-top img { width:72px; height:72px; border-radius:12px; object-fit:cover; border:1px solid var(--border); }
        ...
        .mobile-menu-card { position:absolute; right:0; top:52px; width:220px; padding:10px; border-radius:18px; background:var(--card); border:1px solid var(--border); box-shadow:0 20px 45px rgba(0,0,0,.26); display:grid; gap:6px; z-index:60; }
        ...
        @media(max-width:1100px){
            .chat-shell{grid-template-columns:1fr;height:auto;gap:12px}
            .chat-panel{min-height:420px}
            .contacts{height:280px}
        ...
        @media(max-width:700px){
            ...
            .chat-shell{display:block;height:var(--novaskol-vh,100dvh);min-height:var(--novaskol-vh,100dvh)}
            .chat-panel{border:0;border-radius:0;box-shadow:none;min-height:var(--novaskol-vh,100dvh);background:var(--bg)}
            ...
            body.mobile-chat-open .contact-panel{display:none}
            .mobile-only,.mobile-list-head,.mobile-back-btn,.mobile-menu-btn{display:flex}
            ...
            .contact-search{padding:0 18px 16px;border-bottom:0;background:var(--bg)}
            .contact-search input{border-radius:999px;padding:14px 16px;background:var(--card);border-color:var(--border);font-size:1rem}
            .contacts{height:calc(var(--novaskol-vh,100dvh) - 140px);padding:0 10px 12px;background:var(--bg)}
            .contact{padding:14px 10px;border-bottom:1px solid rgba(255,255,255,.04);border-radius:18px;gap:12px}
            .contact:hover,.contact.active{background:var(--nav-hover)}
            ...
            .chat-head .profile-trigger{color:var(--text)}
            .chat-head .avatar{width:48px;height:48px;border-color:var(--border)}
            ...
            .chat-head-copy{gap:2px}
            .chat-head-copy strong{max-width:none;display:block;font-size:1rem}
            .chat-head-copy .muted{display:flex;font-size:.82rem;color:var(--text-sec)}
...
<main>
    <section class="chat-shell">
        <aside class="chat-panel contact-panel">
            <div class="mobile-list-head">
                ...
                <details class="mobile-top-menu">
                    ...
                    <div class="mobile-menu-card">
                        ...
                        <button type="button" onclick="refreshContacts()"><i class="fa fa-refresh"></i> Actualiser</button>
                    </div>
                </details>
            </div>
            <div class="contact-search"><input id="contactSearch" type="text" placeholder="Rechercher un contact"></div>
            <div class="contacts" id="contacts">
                @foreach ($contacts as $contact)
                    <a class="contact {{ $selectedUserId === (int) $contact->id ? 'active' : '' }}" href="{{ route('modules.chat-prive', ['user' => $contact->id]) }}" data-name="{{ strtolower($contact->nom.' '.$contact->role) }}" data-contact-id="{{ $contact->id }}">
                        <img class="avatar" src="{{ $contact->avatar_url }}" alt="">
                        <span class="contact-main">
                            <strong>{{ $contact->nom }}</strong>
                            <span class="muted"><i class="status-dot {{ $contact->is_online ? 'online' : '' }}"></i> {{ $contact->role }} - {{ $contact->is_online ? 'actif' : 'inactif' }}</span>
                        </span>
                        @if ($contact->unread_count > 0)<span class="unread">{{ $contact->unread_count }}</span>@endif
                    </a>
                @endforeach
            </div>
        </aside>
        <section class="chat-panel conversation-panel">
            @if ($selectedContact && $conversationId)
                <div class="chat-head">
                    <a class="mobile-back-btn" href="{{ route('modules.chat-prive') }}"><i class="fa fa-arrow-left"></i></a>
                    <button class="profile-trigger" type="button" onclick="openProfile()">
                        <img class="avatar" src="{{ $selectedContact->avatar_url }}" alt="">
                        <span class="chat-head-copy">
                            <strong>{{ $selectedContact->nom }}</strong>
                            <span class="muted"><i class="status-dot {{ $selectedContact->is_online ? 'online' : '' }}"></i> {{ $selectedContact->role }} - {{ $selectedContact->is_online ? 'actif' : 'inactif' }}</span>
                        </span>
                    </button>
                    <div class="chat-head-actions">
                        <details class="mobile-top-menu">
                            <summary><i class="fa fa-ellipsis-v"></i></summary>
                            <div class="mobile-menu-card">
                                <button type="button" onclick="openProfile()"><i class="fa fa-user"></i> Voir le profil</button>
                ...
                <div class="typing-line" id="typingLine">{{ $selectedContact->nom }} est en train d'ecrire...</div>
            ...
            @else
                <div class="empty-state">Choisis un contact pour demarrer une discussion privee.</div>
            @endif
        </section>
    </section>
    @if ($selectedContact)
        <div class="modal-backdrop" id="profileModal" onclick="if(event.target===this)closeProfile()">
            <div class="profile-modal">
                <button class="modal-close" onclick="closeProfile()">x</button>
                <div class="profile-top">
                    <img src="{{ $selectedContact->avatar_url }}" alt="">
                    <div><h2>{{ $selectedContact->nom }}</h2><div class="muted">{{ $selectedContact->role }}</div></div>
                </div>
                <div class="profile-line"><strong>Email:</strong> {{ $selectedContact->email }}</div>
                <div class="profile-line"><strong>Derniere activite:</strong> {{ $selectedContact->last_activity ?? 'Non disponible' }}</div>
                <div class="profile-line"><strong>Utilisateur ID:</strong> {{ $selectedContact->id }}</div>
            </div>
        </div>
    @endif
...
const activeContactId = {{ (int) $selectedUserId }};
...
const chatContactsUrl = @json(route('modules.chat-prive.contacts'));
const chatPrivateUrl = @json(route('modules.chat-prive'));
let lastId = 0;
...
function openMediaModal(kind, href, title){
    const modal = document.getElementById('mediaModal');
    const body = document.getElementById('mediaModalBody');
    const heading = document.getElementById('mediaModalTitle');
    const download = document.getElementById('mediaModalDownload');
    if(!modal || !body || !heading || !download) return;
    heading.textContent = title || 'Apercu';
    body.innerHTML = kind === 'image'
        ? '<img src="' + href + '" alt="">'
        : '<iframe src="' + href + '" title="' + escapeHtml(title || 'Fichier') + '"></iframe>';
    download.href = href;
    download.setAttribute('download', title || 'telechargement');
...
function refreshContacts(){loadContacts(document.getElementById('contactSearch')?.value?.trim() || '');}
...
function clearActiveUnreadBadge(){document.querySelector('.contact.active .unread')?.remove();}
function syncMobileViewport(){
    const vh = window.visualViewport ? window.visualViewport.height : window.innerHeight;
    document.documentElement.style.setProperty('--novaskol-vh', vh + 'px');
    setTimeout(()=>keepLatestVisible(), 40);
}
function keepLatestVisible(force=false){
    const box = document.getElementById('messages');
    const input = document.getElementById('messageInput');
    if(!box) return;
    const nearBottom = (box.scrollHeight - box.scrollTop - box.clientHeight) < 140;
    const typing = input && document.activeElement === input;
    if(force || typing || nearBottom) box.scrollTop = box.scrollHeight;
}
function escapeHtml(v){return String(v).replace(/[&<>\"']/g, s => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#039;'}[s]));}

document.getElementById('contactSearch')?.addEventListener('input', e => {
    clearTimeout(contactSearchTimer);
    contactSearchTimer = setTimeout(() => loadContacts(e.target.value.trim()), 260);
});

async function loadContacts(q=''){
    const res = await fetch(chatContactsUrl + '?q=' + encodeURIComponent(q), {headers:{'Accept':'application/json'}});
    const data = await res.json();
    if(!data.success) return;
    const box = document.getElementById('contacts');
    if(!data.contacts.length){
        box.innerHTML='<div class="empty-state" style="height:auto">Aucun contact trouve.</div>';
        return;
    }
    box.innerHTML = data.contacts.map(contactHtml).join('');
    clearActiveUnreadBadge();
}
function contactHtml(c){
    const active = Number(c.id) === activeContactId;
    const online = c.is_online ? 'online' : '';
    const badge = Number(c.unread_count || 0) > 0 ? '<span class="unread">' + c.unread_count + '</span>' : '';
    return '<a class="contact ' + (active ? 'active' : '') + '" href="' + chatPrivateUrl + '?user=' + c.id + '" data-name="' + escapeHtml((c.nom+' '+c.role).toLowerCase()) + '" data-contact-id="' + c.id + '">'
        + '<img class="avatar" src="' + c.avatar_url + '" alt="">'
        + '<span class="contact-main"><strong>' + escapeHtml(c.nom) + '</strong><span class="muted"><i class="status-dot ' + online + '"></i> ' + escapeHtml(c.role) + ' - ' + (c.is_online ? 'actif' : 'inactif') + '</span></span>' + badge
        + '</a>';
}
function messageAttachmentHtml(m){
    if(!m.file_url) return '';
    if(m.type === 'image') return '<a href="' + m.file_url + '" class="js-media-link" data-kind="image" data-title="' + escapeHtml(m.file_name || 'Image') + '"><img class="chat-image" src="' + m.file_url + '" alt=""></a>';
    return '<a class="file-link js-media-link" href="' + m.file_url + '" data-kind="file" data-title="' + escapeHtml(m.file_name || 'Fichier') + '"><i class="fa fa-file"></i>' + escapeHtml(m.file_name || 'Fichier') + '</a>';
}
function messageStatusHtml(m){
    if(m.is_read) return '<span class="meta-status read">&#10003;&#10003;</span>';
    if(m.is_delivered) return '<span class="meta-status delivered">&#10003;&#10003;</span>';
    return '<span class="meta-status sent">&#10003;</span>';
}
function messageHtml(m){
    const file = messageAttachmentHtml(m);
    return '<div class="bubble ' + (m.mine ? 'mine' : '') + '" data-message-id="' + m.id + '" data-mine="' + (m.mine ? '1' : '0') + '" data-content="' + escapeHtml(m.content || '') + '">' 
        + (m.mine ? '' : '<div class="bubble-name">' + escapeHtml(m.sender_name) + '</div>')
        + '<div class="bubble-text">' + escapeHtml(m.content || '') + '</div>'
        + (file ? '<div class="bubble-attachment">' + file + '</div>' : '')
        + '<div class="bubble-meta"><span class="bubble-time">' + escapeHtml(m.created_label || '') + '</span>' + (m.mine ? messageStatusHtml(m) : '') + '</div>'
        + '</div>';
}
function syncMessageNode(node,m){
    node.dataset.content = m.content || '';
    const text = node.querySelector('.bubble-text');
    if(text) text.textContent = m.content || '';
    const attachmentHtml = messageAttachmentHtml(m);
    let attachment = node.querySelector('.bubble-attachment');
    if(attachmentHtml){
        if(!attachment){
            attachment = document.createElement('div');
            attachment.className = 'bubble-attachment';
            node.querySelector('.bubble-meta')?.before(attachment);
        }
        attachment.innerHTML = attachmentHtml;
    } else if(attachment){
        attachment.remove();
    }
    const meta = node.querySelector('.bubble-meta');
    if(meta){
        const time = meta.querySelector('.bubble-time');
        if(time) time.textContent = m.created_label || '';
        const currentStatus = meta.querySelector('.meta-status');
        if(m.mine){
            const temp = document.createElement('div');
            temp.innerHTML = messageStatusHtml(m);
            if(currentStatus) currentStatus.replaceWith(temp.firstElementChild);
            else meta.appendChild(temp.firstElementChild);
...
function renderTyping(users=[]){
    ...
    const label = escapeHtml(users[0]) + " est en train d'ecrire";
    line.innerHTML = '<span>' + label + '</span><span class="typing-dots"><span></span><span></span><span></span></span>';
    line.style.display='flex';
}
async function sendTyping(active){
    if(!conversationId) return;
    if(active === typingActive && active) return;
    typingActive = active;
    try{
        await fetch(chatTypingBaseUrl + '/' + conversationId,{
            method:'POST',
...
function queueTyping(){
    ...
    typingIdleTimer = setTimeout(()=>sendTyping(false), 1400);
}
async function loadMessages(){
    if(!conversationId) return;
    const box = document.getElementById('messages');
    if(!box) return;
    const stickToBottom = !initialMessagesLoaded || (box.scrollHeight - box.scrollTop - box.clientHeight < 120);
    const res = await fetch(chatMessagesBaseUrl + '/' + conversationId, {headers:{'Accept':'application/json'}});
    const data = await res.json();
    if(!data.success) return;
    let appended = false;
    data.messages.forEach(m => {
        lastId = Math.max(lastId, Number(m.id));
        const existing = box.querySelector('.bubble[data-message-id="' + m.id + '"]');
        if(existing){
            syncMessageNode(existing, m);
            return;
        }
...
document.getElementById('composer')?.addEventListener('submit', async e => {
    ...
    const res = await fetch(chatMessagesBaseUrl + '/' + conversationId, {method:'POST', headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json'}, body:form});
    let data = {};
    try { data = await res.json(); } catch (_) {}
    if(res.ok && data.success){
        resetComposerState(e.currentTarget);
        if(data.message){
        ...
        showError(data.message || data.error || 'Envoi impossible pour le moment.');
    }
});
document.getElementById('fileInput')?.addEventListener('change', e => {
    ...
    if(file.type.startsWith('image/')){
        ...
        content.innerHTML = '<span class="file-chip"><i class="fa fa-file"></i> Fichier joint</span>';
    }
...
function openMessageMenu(x,y,bubble){selectedMessage=bubble;document.querySelectorAll('.bubble').forEach(b=>b.classList.remove('context-target'));bubble.classList.add('context-target');const mine=bubble.dataset.mine==='1';document.querySelectorAll('#messageMenu [data-mine-only]').forEach(btn=>btn.style.display=mine?'flex':'none');const menu=document.getElementById('messageMenu');menu.style.left=Math.min(x,window.innerWidth-190)+'px';menu.style.top=Math.min(y,window.innerHeight-150)+'px';menu.style.display='block'}
...
document.getElementById('messageMenu')?.addEventListener('click',async e=>{
    ...
    if(!btn||!selectedMessage)return;
    const id=selectedMessage.dataset.messageId;
    const content=selectedMessage.dataset.content||'';
    ...
    if(btn.dataset.action==='edit'){
        const r=await Swal.fire({title:'Modifier le message',input:'textarea',inputValue:content,showCancelButton:true,confirmButtonText:'Modifier',cancelButtonText:'Annuler'});
        if(!r.isConfirmed)return;
        const res=await fetch(chatMessageBaseUrl + '/' + id,{method:'PUT',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'},body:JSON.stringify({content:r.value})});
        const data=await res.json();
        if(data.success){selectedMessage.querySelector('.bubble-text').textContent=data.message.content;selectedMessage.dataset.content=data.message.content;}
    }
    if(btn.dataset.action==='delete'){
        const ok=await Swal.fire({title:'Supprimer ce message ?',icon:'warning',showCancelButton:true,confirmButtonText:'Supprimer',cancelButtonText:'Annuler'});
        if(!ok.isConfirmed)return;
        const res=await fetch(chatMessageBaseUrl + '/' + id,{method:'DELETE',headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json'}});
        const data=await res.json();
        if(data.success)selectedMessage.remove();
    }
    closeMessageMenu();
});
